...
\begin{document}
  ...
  recorded gestures. This is our list of predefined gestures:
  \begin{itemize}
    \tightlist
    \item Straight line
  ...
  We have come to these conclusions because we have tested all the gestures 30
  times each. We wanted to make sure that the recognition worked well, and we
  therefore did each gesture a lot of times. When we tested the straight line
  and circle gestures, we got a almost 100\% success rate. The squares and
  triangles did not so well, with an success rate of almost 60\%. We found out
  that sometimes, squares would not get recognized a few times in a row. This
  was probably because of the large space that we would make the gesture in,
  resulting in a too large window of points for the recognition to handle.
